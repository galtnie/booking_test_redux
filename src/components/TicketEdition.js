        ...
        return (
            <div className="ticket_edition-main">
                ...
                <div className='ticket_edition-data-row'>
                    ...
                    <div>
                        <input type="text" value={this.state.ticket.title} size="21" className='ticket_edition-input'
                            onChange={e => this.setState({ ticket: {...this.state.ticket, title: e.target.value}   })}
                        />
                    </div>
                </div>
                <div className='ticket_edition-data-row'>
                    ...
                    <div>
                        <input type="datetime-local" placeholder="Disregard to keep unaltered" size="21" className='ticket_edition-input'
                            value={this.state.ticket.from}
                            ...
                            onChange={e => {
                                ...
                                if (start > end) {
                                    ...
                                    this.setState({ ticket: {...this.state.ticket, to: new Date(newToValue).valueOf() }  })
                                }
                                ...
                                this.setState({ ticket: {...this.state.ticket, from: new Date(value).valueOf()}  })
                            }} required />
                    </div>
                </div>
                <div className='ticket_edition-data-row'>
                    ...
                    <div>
                        <input type="datetime-local" placeholder="Disregard to keep unaltered" size="21" className='ticket_edition-input'
                            min={String(new Date().toISOString()).slice(0, -8)}
                            value={this.state.ticket.to}

                            onChange={e => {
                                let value = e.target.value
                                let end = Date.parse(value)
                                let start = Date.parse(this.state.ticket.from)
                                this.setState({ ticket: {...this.state.ticket, to: new Date(e.target.value).valueOf() }  })
                                if (end < start) {
                                    ...
                                    if (String(value).slice(8, 9) === "0" || String(value).slice(-5, -4) === "0") {
                                        setTimeout(() => {
                                            ...
                                            if (end < start) {
                                                ...
                                                this.setState({ ticket: {...this.state.ticket, from: new Date(newFromValue).valueOf() }  })
                                            }
                                        }, 1000)
                                    } else {
                                        let newFromValue = String(new Date(new Date(end + 10800000).setMinutes(0)).toISOString()).slice(0, -8)
                ...
                <div className='ticket_edition-data-row'>
                    <button className="edit-button ui purple button" style={{ background: "#574AE2" }} onClick={(e) => {
                        if (Date.parse(this.state.ticket.from) < Date.parse(new Date())) {
                            e.preventDefault()
                            alert('NEW DATE CANNOT BE IN THE PAST!')
                            return null
                        } else if (this.state.ticket.to < this.state.ticket.from) {
                    ...
                    <button className="edit-button ui button" style={{ background: "#222A68", color: "white" }} onClick={() => { 
                        // this.setState({ editReservation: false }) 
                        }}>
